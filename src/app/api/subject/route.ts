...
export async function POST(req: NextRequest) {
  try {
    ...
    const useEmoji =
      withEmoji || toneOfVoice.toLowerCase() === 'quirky' ? true : false;
    const response = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `You are a helpful email marketing expert that's responsible for creating subject lines that generates high deliverability and open rate. ${
            useEmoji ? 'Use' : 'Do not use'
          } emojis in your answer.`,
        },
        {
          role: 'user',
